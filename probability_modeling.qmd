...
batted_balls$run_consequence <- predict(mod_run_consequence, newdata = batted_balls)

ggplot(batted_balls, aes(x = spray_angle, y = launch_speed, fill = run_consequence)) +
  geom_tile() +
  scale_fill_viridis_c() +
  labs(
...
fit_SAFE_sc_pos <- function(bip_gb, year, pos_position, sc_positioning, bb_space) {
  ...
  for (player in players) {
    ...
    if (player == players[1]) {
      print(paste("Year: ", year, "Position: ", pos_position, "Calculating SAFE for player", player, "with", n_opportunities, "opportunities"))
    }
...
```{r}
ggplot(all_safe_sc_results, aes(x = SAFE, color = position)) +
  geom_density()
```
