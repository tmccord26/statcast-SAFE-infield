...
```{r}
fit_SAFE_sc_pos <- function(bip_data, year, sc_positioning, position, batted_balls, max_retries = 3) {
  ...
  avg_positioning <- sc_positioning |>
  ...
  for (fielder in fielders) {
    ...
    # Get player positioning
    player_pos <- sc_positioning |>
      filter(fielder_id == fielder) |>
      select(batter_hand, pos_x, pos_y, pos_angle)
    
    # Create data with player's actual positioning
    bb_player <- batted_balls |>
      mutate(fielder_id = factor(fielder, levels = levels(play_data$fielder_id))) |>
      left_join(player_pos, by = c("batter_hand"), relationship = "many-to-many") |>
      mutate(
        theta = abs(pos_angle - spray_angle),
    ...
    # Create data with average positioning
    bb_avg <- batted_balls |>
      mutate(fielder_id = factor(fielder, levels = levels(play_data$fielder_id))) |>
      left_join(avg_positioning, by = "batter_hand", relationship = "many-to-many") |>
      rename(pos_angle = avg_pos_angle, pos_x = avg_pos_x, pos_y = avg_pos_y) |>
      mutate(
        theta = abs(pos_angle - spray_angle),
        theta_scaled = (theta - mean(play_data$theta, na.rm = TRUE)) / 
                       sd(play_data$theta, na.rm = TRUE),
    ...
    prob_play_player <- predict(mod_player, newdata = bb_player, type = "response")
    prob_play_avg <- predict(mod_player, newdata = bb_avg, type = "response", re.form = NA)
    
    player_res <- data.frame(
      prob_play_player = prob_play_player,
      prob_play_avg = prob_play_avg,
      run_value = batted_balls$run_value,
      density = batted_balls$density
    ) |>
      mutate(
        prob_play_added = prob_play_player - prob_play_avg,
        SAFE = prob_play_added * run_value * density
      ) |>
      summarize(SAFE = sum(SAFE, na.rm = TRUE) * n_opportunities)
...
for (year in years) {
  ...
  for (position in positions) {
    ...
    sc_pos_year <- sc_positioning |>
      filter(position == !!position, year == !!year) |>
      select(fielder_id, batter_hand, pos_x, pos_y, pos_angle)
    
    safe_results <- fit_SAFE_sc_pos(
      bip_data = bip_gb_year,
      year = year,
      position = position,
      sc_positioning = sc_pos_year,
      batted_balls = batted_balls
    )
