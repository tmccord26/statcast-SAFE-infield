...
## Rethinking SAFE: Estimate over all batted balls rather than a synthetic space

This means that we can just iterate over all batted balls rather than creating a synthetic space weighted by density. 
Then, we don't need to calculate density.
Then, for a given ground ball, we have two probabilities and an estimated run value. 
