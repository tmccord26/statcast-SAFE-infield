...
The resulting dataset contains 21 variables from 145070 ground balls hit into fair territory from the 2023-2025 seasons. 
...
Our paper first estimates the probability that a fielder makes a successful play on a batted ball for each combination of year and position.
...
Each play is associated with a change in expected runs for the offensive team, which we denote as $\Delta_{RE}$.
This value is calculated as the difference in run expectancy before and after the play.
We use a generalized additive model (GAM) to estimate $\Delta_{RE}$ of a ground ball $j$ based on its launch speed $V_j$ and spray angle $\theta_j$:
...
$$
SAFE_{i,p,y} = \sum_{\theta} \sum_{V} PPA_{i,p,y}(\theta, V) \times \Delta_{RE}(\theta, V)
$$
