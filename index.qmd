...
We evaluate our models by verifying that they produce reasonable results and by comparing our results to established defensive metrics such as Defensive Runs Saved (DRS) and Outs Above Average (OAA).
...
-   **out\_{pos}**: A binary indicator for each infielder position (1B, 2B, SS, 3B) indicating whether or not the player at that position recorded a successful play on the batted ball.
...
This plot shows which types of plays are resulting in the most contributions to SAFE for Gimenez at a velocity of 90 mph.
The area under this curve represents the total SAFE value for Gimenez at a velocity of 90 mph.
In practice, we integrate over all plausible velocities to get the final SAFE value.
...
We focus the evaluation of our model on the production of the SAFE model without player-specific positioning.
...
For this reason, the effect of small sample size on SAFE values is less pronounced than it would be if we included players with far fewer opportunities.
...
We compare our estimates to two state-of-the-art public defensive metrics, Defensive Runs Saved (DRS) and Outs Above Average (OAA).
It's important to note that the SAFE metric only considers ground balls hit into fair territory, while DRS and OAA consider all batted balls.
Also, DRS does not take an "above average" approach to run-preventation, while OAA does, but with outs instead of runs.
...
Although we were not able to implement the full Bayesian hierarchical probit regression model from @jensen_2009 due to computational constraints, we were able to fit a frequentist hierarchical probit regression model that produced reasonable results.
